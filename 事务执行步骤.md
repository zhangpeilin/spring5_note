...
### 1、代理类开始调用事务方法invokeWithinTransaction()
...
#### 1.1、必要时创建事务createTransactionIfNecessary()
...
#### 1.4、绑定ConnectionHolder到当前线程bindResource()
...
#### 1.5、封装事务相关对象prepareTransactionInfo()
...
#### 1.6、已存在事务处理handleExistingTransaction()
...
#### 3.2、jdbcTemplate获取链接doGetConnection()
...
#### 3.3、jdbcTemplate关闭链接doReleaseConnection()
...
#### 4.4、事务完成后清理&恢复挂起的事务cleanupAfterCompletion()
...
#### 4.5、事务完成后恢复链接配置resetConnectionAfterTransaction()
